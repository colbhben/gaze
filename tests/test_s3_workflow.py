...
class S3WorkflowTests(unittest.TestCase):
    ...
    def test_serial_download_backup_dry_run_uses_unprocessed_prefix(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(operation["upload"]["transport"], "file_mount")
            self.assertEqual(operation["upload"]["operation"], "copy")
            self.assertIn("/nfs/gaze/unprocessed/aea/loc5_script4_seq6_rec1/mps_eye_gaze/", operation["upload"]["target_path"])
